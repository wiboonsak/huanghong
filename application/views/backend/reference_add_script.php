<script>
	...
	function removeMe(imageID,imageName){
		 swal({
                ...
                buttonsStyling: false
		   		...
		   		$.post('<?php echo base_url('Control/deletereferenceImg')?>', { imageID : imageID , imageName:imageName }, function(data){ 
					console.log(data);
					   if(data=='1'){
						     swal({
								title: 'Deleted !',
	...
	function AddNews(){  // news_title  news_detail
		...
		if(news_title==''){
			swal({
					title: 'warning !',
					text: "Please enter a news topic.",
					type: 'warning',
					confirmButtonClass: 'btn btn-confirm mt-2'
			}) 
				...
				swal({
					title: 'warning !',
					text: "Please enter news details.",
					type: 'warning',
					confirmButtonClass: 'btn btn-confirm mt-2'
			}) 
		}else{
		  ...
		  // console.log("File Uploaded");
			...
			if(status=='success'){
				...
				swal({
                     ...
                     setTimeout(function(){ window.location.href = "<?php echo base_url('Control/reference_add/')?>"+data; }, 2000);
			}else{
					 swal({
		   		...
		   		$.post('<?php echo base_url('Control/deleteyoutube')?>', { dataID : dataID , table : table }, function(data){  
					console.log(data);
					   if(data=='1'){
						  ...
						  setTimeout(function () {
                        window.location.href = "<?php echo base_url('Control/reference_add/') ?>"+currentID ;
                    }, 2000);
