			...
			<div class="row">
				<div class="col-md-12 text-center">
					<div class="inner-banner-detail">
						<p style="padding-bottom: 10px; text-align: left;"><a href="<?php echo base_url('Welcome')?>">Home</a><span> / </span><a href="<?php echo base_url('Service')?>">Services</a><span> / </span><?php echo $topic?></p>
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-md-12 col-sm-12 col-xs-12 m-t-35">
                              <?php
  $limit ='';                           
$countAll=$this->Product_model->getservicebycate($limit,$id,'-100','-100');

        $countRow = $countAll->num_rows(); 
        $totalRow = ceil($countRow/$perpage);
foreach ($getservicebycate->result() AS $data) {
    $firstImg = $this->Product_model->getserviceimg($data->id);
    ?>
				<div class="row p-b-35">
					<div class="col-md-4 col-sm-6 col-xs-12">
						<div class="logistic_news">
							<img src="<?php echo base_url('uploadfile/service/') . $firstImg ?>" alt="image" >
						</div>
					</div>
					<div class="col-md-8 col-sm-6 col-xs-12">
						<div class="logistic_news">
							<div class="logistic_news_detail">
								<h3><a href="<?php echo base_url('Service/service_detail/').$data->id ?>"><?php echo  substr_replace($data->service_topic,'....',35) ?></a></h3>
								<?php echo $this->Product_model->str_limit_html($data->service_detail,300)?>
								<a href="<?php echo base_url('Service/service_detail/').$data->id ?>" class="btn btn-info">Read More</a>
								<span style="float: right"><i class="fa fa-calendar" aria-hidden="true"></i> <?php echo $this->Product_model->getDayMonthYear($data->date)?></span>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="col-md-12 text-center padding-top-50">
					<nav aria-label="...">
                                             <?php 
                                                    ...
                                                    ?>
						<ul class="pagination">
							<li <?php if($page == 1){?>class="isDisabled"<?php }?>> <a href="<?php echo base_url('Service/Page/').$topic.'/'.$id.'/'.$page2?> " >
								<span>
                      <span aria-hidden="true">Prev</span>
							...
							<li class=" <?php if($page == $i){ echo 'active'; }?>">
                                                             <a href="<?php echo base_url('Service/Page/').$topic.'/'.$id.'/'.$i?>">
								<span><?php echo $i?><span class="sr-only">(current)</span></span>
                                                                </a>
							</li>
<?php }?>     
                                                        <li <?php if($totalRow <= 1){?>class="isDisabled"<?php }?>>
                                                            <a href="<?php echo base_url('Service/Page/').$topic.'/'.$id.'/'.$page3?>" >
								<span>
